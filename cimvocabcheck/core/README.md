...
`cimvocabcheck-core` answers the question *"does this query (or shapes graph) make sense
...
The `cimnotebook/vscode` module ships a VS Code extension that runs the language
...
npm install
npm run copy-jar    # copies cimvocabcheck-lsp.jar into server/
npm run bundle      # compiles TypeScript and bundles with esbuild
npx vsce package    # produces cimnotebook-<version>.vsix
```

The `npm run copy-jar` step copies `cimvocabcheck/lsp/target/cimvocabcheck-lsp.jar`
into `cimnotebook/vscode/server/`. The JAR is bundled inside the VSIX so users do
...
```bash
code --install-extension cimnotebook-X.Y.Z.vsix
```
...
**Point CIMVocabCheck at a schema** for full validation. There are two ways: an `opencgmes.json`
config, or a `# [endpoint=...]` directive in the query (a SPARQL endpoint that hosts the
schema). With neither, CIMVocabCheck performs a **syntax-only** check — there is no bundled
default schema.

Add an `opencgmes.json` file: all CIMVocabCheck settings live under a top-level `"cimvocabcheck"`
section (so `opencgmes.json` can host config for other OpenCGMES tools too). The file is
**discovered git-style**: CIMVocabCheck walks up the directory tree from each open file and uses
the nearest `opencgmes.json`, so different subtrees can use different configs. JSON comments
(`//`) and trailing commas are allowed.

Generate a commented starter file with the **CIMNotebook: Create Config File** command (or
`cimvocabcheck init` from the CLI).
...
| --- | --- | --- |
| `cimnotebook.serverJar` | *(bundled)* | Absolute path to a custom `cimvocabcheck-lsp.jar`. Leave empty to use the JAR bundled inside the extension. |
| `cimnotebook.javaExecutable` | `java` | Java executable used to launch the server. Must be Java 21 or later. |
| `cimnotebook.javaArgs` | `[]` | Extra JVM arguments, e.g. `["-Xmx512m"]`. |
| `cimnotebook.trace.server` | `off` | LSP trace level: `off`, `messages`, or `verbose`. Useful for debugging. |
...
1. Run **CIMNotebook: Show Output** from the Command Palette to open the
   output channel — startup errors and schema-load failures are logged there.
2. Check that Java 21+ is on `PATH`, or set `cimnotebook.javaExecutable` explicitly.
3. With no config (or endpoint) validation is syntax-only; for full validation verify your
   `opencgmes.json` `cimvocabcheck` section points to valid schema files.
4. Set `cimnotebook.trace.server` to `messages` to see raw LSP traffic.

---

## Language server

`cimvocabcheck/lsp` is a standalone [LSP 3.17](https://microsoft.github.io/language-server-protocol/)
server that wraps the `cimvocabcheck-core` library. It communicates over `stdio` and can be
...
`cimvocabcheck/cli` ships a command-line tool for validating queries and shapes in CI pipelines.
...
```bash
java -jar cimvocabcheck-cli.jar --endpoint http://localhost:3030/cgmes/query path/to/query.rq
```
...
The entry point is [`SparqlValidationApi`](src/main/java/de/soptim/opencgmes/cimvocabcheck/core/SparqlValidationApi.java).
Construct it with any [`SchemaIndex`](src/main/java/de/soptim/opencgmes/cimvocabcheck/core/schema/SchemaIndex.java);
[`RdfsSchemaIndex`](src/main/java/de/soptim/opencgmes/cimvocabcheck/core/schema/RdfsSchemaIndex.java)
...
or `?s a ?cls`), CIMVocabCheck cannot perform per-triple existence or domain/range checks. It emits
...
`sh:PropertyShape`'s `sh:path`. CIMVocabCheck resolves this automatically: when a
...
CIMVocabCheck performs **two distinct passes** over a SHACL shapes graph.
