 ...
 *
 * <p>When an embedded SPARQL constraint lives inside a shape with {@code sh:targetClass},
 * the validator synthesises a {@code ?this rdf:type <targetClass>} pattern so that
 * domain/range checks fire correctly for {@code $this}.</p>
 *
 * <p>The schema used here is deliberately richer (includes domain maps) so that
 * {@link SparqlValidationCode#PROPERTY_NOT_ALLOWED_FOR_CLASS} can fire.</p>
 */
public class ShaclThisBindingTest {
    ...
    private static final String PROP_NOMINAL_V   = CIM + "VoltageLevel.nominalVoltage"; // domain=VoltageLevel
    ...
    public void targetClassIsResolvedFromEnclosingShape() {
        ...
        boolean found = queries.stream()
                .anyMatch(q -> q.targetClasses().contains(
                        NodeFactory.createURI(CLASS_AC_LINE)));
    ...
    public void queryWithNoTargetShape_hasEmptyTargetClasses() {
        ...
        boolean found = queries.stream()
                .anyMatch(q -> q.rawQuery().contains("?this a cim:ACLineSegment")
    ...
    public void correctPropertyForTargetClass_noError() {
        ...
        boolean spurious = r.embeddedResults().stream()
    ...
    public void wrongPropertyForTargetClass_emitsDomainError() {
        ...
        // After injection: ?this typed as ACLineSegment → domain of nominalVoltage is VoltageLevel → MISMATCH.
        Graph g = loadShapes("shacl/shapes-this-binding.ttl");
        ShaclValidationResult r = api.validateShacl(g);

        boolean found = r.embeddedResults().stream()
                .filter(er -> er.embedded().rawQuery().contains("VoltageLevel.nominalVoltage")
                           && er.embedded().targetClasses().contains(NodeFactory.createURI(CLASS_AC_LINE)))
                .flatMap(er -> er.result().annotations().stream())
                .anyMatch(a -> a.code() == SparqlValidationCode.PROPERTY_NOT_ALLOWED_FOR_CLASS
                            && a.term().getURI().equals(PROP_NOMINAL_V));
        assertTrue("wrong-domain property for $this must produce PROPERTY_NOT_ALLOWED_FOR_CLASS", found);
    }
    ...
    public void noTargetClass_domainCheckSkipped_noImpliedTypeInfo() {
        ...
        // TargetQueryShape's SELECT references only ACLineSegment (type assertion only) — no domain mismatch.
        boolean domainError = r.embeddedResults().stream()
                .filter(er -> er.embedded().targetClasses().isEmpty()
    ...
    public void targetClassViaPropertyShape_isResolved() {
        // ViaPropShape: sh:targetClass is on the NodeShape, the query is inside sh:property -> sh:sparql.
        ...
        boolean found = queries.stream()
                .anyMatch(q -> q.rawQuery().contains("VoltageLevel.nominalVoltage")
    ...
    public void targetClassViaPropertyShape_domainErrorFires() {
        // ViaPropShape uses nominalVoltage (domain=VoltageLevel) on $this (typed as ACLineSegment) → error.
        Graph g = loadShapes("shacl/shapes-this-binding.ttl");
        ShaclValidationResult r = api.validateShacl(g);

        boolean found = r.embeddedResults().stream()
                .filter(er -> er.embedded().rawQuery().contains("$this cim:VoltageLevel.nominalVoltage ?bad"))
                .flatMap(er -> er.result().annotations().stream())
                .anyMatch(a -> a.code() == SparqlValidationCode.PROPERTY_NOT_ALLOWED_FOR_CLASS);
    ...
    private static Graph loadShapes(String resourcePath) {
        try (InputStream in = ShaclThisBindingTest.class.getClassLoader()
