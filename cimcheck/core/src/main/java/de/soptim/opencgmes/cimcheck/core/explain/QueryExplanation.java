 ...
 * compiles it and as Jena's static optimizer rewrites it. This is the CIMcheck equivalent of
 * {@code arq.qparse --print=query,op,opt} — no data is executed.
 *
 * <p>When the input is not a query (for example a SPARQL Update, which has no algebra plan) or could
 * not be parsed, {@link #algebra()} and {@link #optimizedAlgebra()} are {@code null} and
 * {@link #note()} carries a human-readable explanation. Use {@link #ofMessage} for that case.</p>
 *
    ...
    public String render() {
        ...
        if (optimizedAlgebra != null) {
            sb.append('\n').append("# Algebra (optimized)\n")
              .append(optimizedAlgebra.stripTrailing()).append('\n');
        }
        return sb.toString();
    }
}
