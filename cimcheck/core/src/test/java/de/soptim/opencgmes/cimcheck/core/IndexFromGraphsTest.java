...
/**
 * Covers {@link CgmesSchemaLoader#indexFromGraphs} — building a schema index from in-memory
 * graphs, as used when loading a CGMES schema from a SPARQL endpoint's named graphs.
 */
public class IndexFromGraphsTest {
    ...
    /**
     * A minimal CIM 17 profile graph WITHOUT a {@code cim} namespace prefix — exactly what a
     * graph fetched over SPARQL looks like. Profile detection is prefix-based, so this only
     * wraps if {@code indexFromGraphs} re-asserts the prefix from the CIM100 IRIs.
     */
    private static Graph profileGraphWithoutCimPrefix() {
        ...
        assertTrue("test graph must not declare a cim prefix",
    ...
    public void buildsIndexFromPrefixlessProfileGraph() throws Exception {
        ...
        assertFalse("ACLineSegment class should be in the index",
    ...
    public void skipsNonProfileGraphsButLoadsProfiles() throws Exception {
        ...
        RdfsSchemaIndex index = CgmesSchemaLoader.indexFromGraphs(
