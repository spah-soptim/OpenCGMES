...
/**
 * Tests for {@link SparqlValidationApi#inferProfileScope(Graph)} and the updated
 * {@link SparqlValidationApi#validateShacl(Graph)} (which now uses inferred scope instead of
 * all-profiles).
 */
public class InferProfileScopeTest {
    ...
    public void inferScope_emptyGraph_returnsAllProfiles() {
        ...
        assertEquals("fallback: all three profiles",
                3, scope.size());
    ...
    public void inferScope_unknownTermsOnly_returnsAllProfiles() {
        ...
        assertEquals("unknown-terms-only → fallback to all profiles",
                3, scope.size());
    }
    ...
    public void inferScope_embeddedSparqlTermsContribute() {
        ...
        assertTrue("EQ must be in scope (from sh:targetClass)",
                scope.contains(VersionIri.of(PROFILE_EQ)));
        assertTrue("TP must be in scope (from embedded SPARQL)",
                scope.contains(VersionIri.of(PROFILE_TP)));
    }
    ...
    public void validateShacl_noArgUsesInferredScope() {
        ...
        assertTrue("EQ must be in selectedProfiles",
                a.selectedProfiles().contains(VersionIri.of(PROFILE_EQ)));
    }
    ...
    public void validateShacl_dyOnlyTerms_fallsBackToAllProfiles() {
        ...
        boolean dyInSelectedProfiles = result.shapeAnnotations().stream()
